class Server:
    ...
    def dataset(self) -> List[List]:
        ...
        if self.__dataset is None:
            with open(self.DATA_FILE) as file:
                ...
                dataset_ = [rw for rw in reader]
            self.__dataset = dataset_[1:]
    ...
    def indexed_dataset(self) -> Dict[int, List]:
        ...
        if self.__indexed_dataset is None:
            ...
            truncated_dataset = dataset_[:1000]
            self.__indexed_dataset = {
                i: dataset_[i] for i in range(len(dataset_))
            }
    ...
    def get_hyper_index(self, index: int = None, page_size: int = 10) -> Dict:
        ...
        """
        focus = []
        dataset_ = self.indexed_dataset()
        index = 0 if index is None else index
        keys = sorted(dataset_.keys())
        assert index >= 0 and index <= keys[-1]
        [focus.append(i)
         for i in keys if i >= index and len(focus) <= page_size]
        data = [dataset_[v] for v in focus[:-1]]
